...
if 'por_numero' not in st.session_state:
    st.session_state.por_numero = {n: deque(maxlen=20) for n in range(37)}
...
for i in range(1, len(st.session_state.historico)):
    ...
    if ant in numeros_proibidos and atual in numeros_proibidos[ant]:
        ...
        st.session_state.por_numero[ant].append("X")
        st.session_state.reflexiva_seq.append("<span style='color:red;'>X</span>")
    else:
        resultado_reflexivo[i - 1] = "1"
        st.session_state.por_numero[ant].append("1")
        ult = ''.join([c for c in st.session_state.reflexiva_seq[-1:] if 'X' not in c])
        if ult and ult[-1].isdigit():
            novo = int(ult[-1]) + 1
        else:
            novo = 1
        st.session_state.reflexiva_seq.append(str(novo))
...
def mostrar_resultados(coluna, inicio, fim):
    ...
    with coluna:
        for n in range(inicio, fim + 1):
            ultimos = list(st.session_state.por_numero[n])
            alert_style = ""
...
html_seq = ''.join(st.session_state.reflexiva_seq)
wrapped_seq = '<br>'.join([html_seq[i:i+100] for i in range(0, len(html_seq), 100)])
st.markdown(f"<div style='word-wrap: break-word;'>{wrapped_seq}</div>", unsafe_allow_html=True)
...
if len(st.session_state.historico) >= 5:
    ...
    for i in range(2, len(st.session_state.historico)):
        ...
        if len(set([d1, d2, d3])) == 3 and len(set([c1, c2, c3])) == 3:
            alternancia.append((i, st.session_state.historico[i]))

    if alternancia:
        for idx, num in alternancia[-5:]:
            st.warning(f"Alternância dupla detectada em {idx}: número {num}")
            st.audio("https://www.soundjay.com/button/beep-07.wav", format="audio/wav")
